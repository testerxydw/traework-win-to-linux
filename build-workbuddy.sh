...
# 自适应脚本位置：
#   - 位于 workbuddy-build/scripts/ 时：ROOT_DIR=workbuddy-build，SRC_DIR=项目根
#   - 位于项目根/ 时：ROOT_DIR=项目根/workbuddy-build，SRC_DIR=项目根
if [[ "$(basename "${SCRIPT_DIR}")" == "scripts" && -d "${SCRIPT_DIR}/../wb_pkg" ]]; then
    ROOT_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"      # workbuddy-build
    SRC_DIR="$(cd "${ROOT_DIR}/.." && pwd)"         # 项目根（源材料默认放这里）
else
    SRC_DIR="${SCRIPT_DIR}"                         # 脚本在项目根
    ROOT_DIR="${SRC_DIR}/workbuddy-build"           # wb_pkg 与产物放这里
fi
